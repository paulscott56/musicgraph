...
/**
 * In MusicGraph terminology, a work is a distinct intellectual or artistic creation, 
 * which can be expressed in the form of one or more audio recordings. 
 * 
 * While a work in MusicGraph is usually musical in nature, it is not necessarily so. 
 * For example, a work could be a novel, play, poem or essay, later recorded as an oratory or audiobook.
 * 
 * Distinctiveness
 * A work’s distinctiveness is based on the artists who contributed to its final output, and whether a work is 
 * derived from another original work. 
 ...
 * 
 *  Discrete works
 *  An individual song, musical number or movement. 
 *  This includes recitatives, arias, choruses, duos, trios, etc. 
 *  In many cases, discrete works are a part of larger, aggregate works.
 *  
 *  Aggregate works
 *  An ordered sequence of one or more songs, numbers or movements, such as: 
 *  symphony, opera, theatre work, concerto, and concept album. 
 *  A popular music album is not considered a distinct aggregate work unless it is 
 *  evident that such an album was written with intent to have a specifically ordered sequence of 
 *  related songs (i.e. a “concept album”).
 *   
 * @author paul
 *
 */
